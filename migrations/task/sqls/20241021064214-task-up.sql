...
FROM 
    (SELECT user_id, SUM(purchased_credits) as total_credit FROM "CREDIT_PURCHASE"
    WHERE user_id = (SELECT id FROM "USER" WHERE email = 'wXlTq@hexschooltest.io')
    GROUP BY user_id) AS "CREDIT_PURCHASE"
INNER JOIN
    (SELECT user_id, COUNT(id) as used_credit FROM "COURSE_BOOKING"
    WHERE 
	user_id = (SELECT id FROM "USER" WHERE email = 'wXlTq@hexschooltest.io')
	AND 
	join_at IS NOT NULL
...
SELECT "SKILL".id, COUNT("SKILL".id) AS coach_total, "SKILL".name AS 專長名稱
FROM 
	"COACH_LINK_SKILL"
INNER JOIN
    "SKILL"
...
GROUP BY "SKILL".id
ORDER by COUNT("COACH_LINK_SKILL".skill_id) DESC
LIMIT 1;
...
FROM 
	"CREDIT_PURCHASE"
WHERE 
    "CREDIT_PURCHASE".purchase_at BETWEEN '2024-11-01' AND '2024-11-30';
